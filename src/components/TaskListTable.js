...
const TaskListTable = () => {
    ...
    useEffect(() => {
        if (tasks.taskRemoved !== null) {
            toast.success(`Tarefa ${tasks.taskRemoved.description} excluída com sucesso!`, 
                { position: toast.POSITION.TOP_CENTER});
            tasks.clearTaskRemoved();
        }

        if (tasks.taskUpdated !== null) {
            toast.success(`Tarefa ${tasks.taskUpdated.description} foi marcada como ${!tasks.taskUpdated.done ? "não" : ""} concluída!`, 
                { position: toast.POSITION.TOP_CENTER});
